...
if(isset($_GET['id'])) {
	...
	if(mysqli_query($conn, $sql)) {
		...
		if($sql2->execute()) {
			echo "<script>alert('Success!');window.location.href='document-verification.php?application=$app&user=$LoggedUserID&appID=$appID'</script>";
		}

	} else {
		echo "<script>alert('Error!');window.location.href='document-verification.php?application=$app&user=$LoggedUserID&appID=$appID'</script>";
	}
}
