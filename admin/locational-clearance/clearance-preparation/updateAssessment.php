...
if(isset($_POST['updateAssessment'])) {
	...
	if($sql->execute()) {
		...
		$sql2->execute();
		echo "<script>alert('Successful update!');window.location.href='../clearance-preparation/?slug=$slug'</script>";
	} else {
		$error = 'Error: '. $conn->error;
		$error_explode = explode("'", $error);
		$error_implode = implode("", $error_explode);
		echo "<script>alert('$error_implode');window.location.href='../clearance-preparation/?slug=$slug'</script>";
	}
