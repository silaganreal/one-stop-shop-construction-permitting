...
if(isset($_POST['updateArea'])) {
		...
		if($sql->execute()) {
			...
			if($num6 > 0) {
				...
				if($notifStat === 'N') {
					...
					$sql4->bind_param("ssss", $newStat, $dateIn, $userappID, $onlineappID);
					$sql4->execute();

					$sql7 = $conn->prepare("INSERT INTO notifications(userappID, onlineappID, dateIn) VALUES(?,?,?)");
					$sql7->bind_param("sss", $userappID, $onlineappID, $dateIn);
					$sql7->execute();
					echo "
						<script>
							alert('Status has been updated. Forwarded to next step.');
							window.location.href='$location2?application=$onlineappID&user=$LoggedUserID&appID=$userappID'
						</script>
					";
				} else {
	...
	if($areaStatus == 'NO') {
		...
		$sql = $conn->prepare("UPDATE onlineapplications SET currentArea=?, applicationRemarks=? WHERE id=?");
		$sql->bind_param("sss", $currentArea, $areaRemarks, $onlineappID);
	...
	if($areaStatus == 'RETURN') {
		$newCurrentArea = 'Returned';
		$sql = $conn->prepare("UPDATE onlineapplications SET currentArea=?, applicationRemarks=? WHERE id=?");
		$sql->bind_param("sss", $newCurrentArea, $areaRemarks, $onlineappID);
		...
		if($sql->execute()) {
			...
			if($num6 > 0) {
				...
				$isCurrentArea = 'N';
				$isFinished = 'N';
				$sql2 = $conn->prepare("UPDATE tracking SET dateIn=?, dateReceived=?, isCurrentArea=?, isFinished=?, remarks=? WHERE onlineappID=? AND area LIKE ?");
				$sql2->bind_param("sssssss", $dateIn, $dateIn, $isCurrentArea, $isFinished, $areaRemarks, $onlineappID, $currentArea);
				$sql2->execute();
				...
				if($notifStat === 'N') {
					...
					echo "
						<script>
							alert('Status has been updated. Returned to Client.');
							window.location.href='$location2?application=$onlineappID&user=$LoggedUserID&appID=$userappID'
						</script>
					";
				} else {
					...
					$sql5->bind_param("sss", $userappID, $onlineappID, $dateOut);
					$sql5->execute();
					echo "<script>alert('Status has been updated. Returned to Client.');window.location.href='$location2?application=$onlineappID&user=$LoggedUserID&appID=$userappID'</script>";
				}
			} else {
				$sql6 = $conn->prepare("INSERT INTO notifications(userappID, onlineappID, dateIn) VALUES(?,?,?)");
				$sql6->bind_param("sss", $userappID, $onlineappID, $dateOut);
