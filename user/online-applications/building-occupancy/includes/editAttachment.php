...
if(isset($_POST['btnUpdateAttachment'])) {
	...
	$fileApplication = $_POST['fileApplication'];
	...
	if($sql->execute()) {
		echo "<script>alert('Attachment has been updated!');window.location.href='../document-verification.php?application=$fileApplication&user=$fileUser&appID=$fileAttachAppID'</script>";
	} else {
		$error = "Error: ". $conn->error;
		$error_explode = explode("'", $error);
		$error_implode = implode("", $error_explode);
		echo "<script>alert('$error_implode');window.location.href='../document-verification.php?application=$fileApplication&user=$fileUser&appID=$fileAttachAppID'</script>";
	}
