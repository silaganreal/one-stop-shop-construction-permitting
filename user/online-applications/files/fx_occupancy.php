...
function addNewApplication($userID, $appID, $appType, $appDate, $appTime, $currentArea, $appStatus, $slug) {
        ...
        if($sql2->execute()) {
                ...
                $fdir = 'OCCUPANCY_PERMIT/'. $directory;
                ...
                if( !empty($OCC1) ) {
                    ...
                    foreach($img_desc as $val)
                    {
                                ...
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC1_fileName, $OCC1_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                    }
                }
                //-------------------------------
                ...
                if( !empty($OCC2) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC2_fileName, $OCC2_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC3) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC3_fileName, $OCC3_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC4) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC4_fileName, $OCC4_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
               //-------------------------------
                ...
                if( !empty($OCC5) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC5_fileName, $OCC5_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC6) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC6_fileName, $OCC6_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC7) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC7_fileName, $OCC7_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC8) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC8_fileName, $OCC8_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC9) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC9_fileName, $OCC9_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC10) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC10_fileName, $OCC10_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC11) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC11_fileName, $OCC11_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC12) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC12_fileName, $OCC12_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC13) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC13_fileName, $OCC13_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if( !empty($OCC14) ) {
                        ...
                        foreach($img_desc as $val) {
                                ...
                                $sql = $conn->prepare("INSERT INTO attachments(userID, applicationID, name, file, status, remarks, folderDirectory) VALUES(?,?,?,?,?,?,?)");
                                $sql->bind_param("sssssss", $userID, $lastID, $OCC14_fileName, $OCC14_new_name, $status, $remarks, $fdir);
                                $sql->execute();
                        }
                }
                //attachement-------------------------------------------------------------------------------------------------------
                ...
                if(mysqli_multi_query($conn, $sql_track)) {
                        ...
                        $sql_trackplan .= "INSERT INTO tracking_plan_evaluation(onlineappID, area, incharge, initial, timeIn, dateIn, timeOut, dateOut, remarks) VALUES('$lastID','Structural','Engr. Filemon Tandinco III','','','','','','');";
                        ...
                        if(mysqli_multi_query($conn, $sql_trackplan)) {
                                ...
                                $sql_trackassess .= "INSERT INTO tracking_assessment(onlineappID, assessedFees) VALUES('$lastID','Line And Grade');";
                                ...
                                $sql_trackassess .= "INSERT INTO tracking_assessment(onlineappID, assessedFees) VALUES('$lastID','Plumbing');";
